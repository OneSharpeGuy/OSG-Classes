...
class EZSettings {
  ...
  <#
      <!-- [Load]
      Alternative factory method, sort of an Alias for [EZSettings]::new()
      (Same as Retrieve)
      -->
  ...
  <#
      <!-- [Fetch]
      Construct [EZSettings] and return a hashtable
      -->

  #>
  
  static [Hashtable] Fetch($pathHint) {
    return $([EZSettings]::new($pathHint)).ToHashTable()		
